...
export default function VoiceVisualizer({ state }: Props) {
  ...
  const createParticle = useCallback(
    (cx: number, cy: number, radius: number): Particle => {
      ...
      return {
        ...
        hue: state === 'listening' ? 180 + Math.random() * 40 : 245 + Math.random() * 20,
        life: 0,
  ...
  useEffect(() => {
    ...
    if (particlesRef.current.length === 0) {
      for (let i = 0; i < 120; i++) {
        particlesRef.current.push(createParticle(cx, cy, baseRadius));
      }
    }
    ...
    const animate = () => {
      ...
      if (state === 'listening' || state === 'processing') {
        ...
        for (let i = 0; i < barCount; i++) {
          ...
          const amplitude =
            (Math.sin(t * 3 + i * 0.4) * 0.5 + 0.5) *
            (Math.cos(t * 1.7 + i * 0.2) * 0.3 + 0.7) *
            baseRadius *
            0.15 *
            intensity;
          ...
          const barAlpha = 0.3 + (amplitude / (baseRadius * 0.15)) * 0.5;
          ctx.strokeStyle =
            state === 'listening'
              ? `rgba(0, 229, 255, ${barAlpha})`
              : `rgba(108, 99, 255, ${barAlpha})`;
          ctx.lineWidth = 2;
          ctx.beginPath();
          ctx.moveTo(x1, y1);
          ctx.lineTo(x2, y2);
      ...
      for (let i = particles.length - 1; i >= 0; i--) {
        ...
        // Move towards/away from center based on state
        const dx = cx - p.x;
        const dy = cy - p.y;
        ...
        const alpha = p.opacity * (1 - lifeRatio) * intensity;
        ...
        if (p.life > p.maxLife || dist > baseRadius * 3) {
          particles[i] = createParticle(cx, cy, baseRadius);
        }
      }

      // Spawn new particles when listening
      if (state === 'listening' && particles.length < 200) {
