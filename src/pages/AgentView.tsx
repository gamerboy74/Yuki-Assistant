    ...
    <div className="absolute top-10 right-10 z-[60]">
      <div className="relative">
        ...
        >
          ...
          <span className={`material-symbols-outlined text-white/20 text-[16px] transition-transform duration-300 ${showMenu ? 'rotate-180' : ''}`}>
            expand_more
          </span>
        </button>
    ...
    <div className="absolute bottom-0 inset-x-0 p-8 pt-20 bg-gradient-to-t from-black via-black/90 to-transparent pointer-events-none z-50">
      <div className="max-w-2xl mx-auto pointer-events-auto">
        <div className="flex items-center gap-2 mb-2 px-4 opacity-50">
          ...
          <span className="text-[9px] font-mono tracking-[0.2em] uppercase text-white/40">Status: Online // Link: {activeLabel} // Cycle: {orbState.toUpperCase()}</span>
        </div>
            ...
            <div className="flex items-center gap-[2px] h-3 px-1">
              {[...Array(5)].map((_, i) => (
                <div
                  ...
                  style={{ background: activeColor, animationDelay: `${i * 0.1}s`, height: orbState === 'listening' ? '12px' : orbState === 'processing' ? '8px' : '4px' }}
                />
              ))}
            </div>
          </div>
...
export default function AgentView({
  ...
  return (
    <div className="relative flex flex-col h-full overflow-hidden bg-transparent text-white selection:bg-cyan-500/30">
      ...
      {showChat && (
        <div ref={threadRef} className="flex-[0.7] overflow-y-auto px-6 pb-36 pt-4 border-t border-white/5 bg-black/40 backdrop-blur-xl" style={{ animation: 'slideUp 0.4s cubic-bezier(0.2,0.8,0.2,1) both' }}>
          <MessageThread messages={messages} orbState={orbState} />
        </div>
      )}
      ...
      <NeuralTerminal 
        ...
        activeLabel={activeProvider.label.toUpperCase()} 
